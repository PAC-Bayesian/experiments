...
class PriorPlots(object):
    def __init__(self, prior, figsize=plt.rcParams['figure.figsize'], save=False, path='./'):
        self.prior = prior
        if isinstance(prior, AdditivePrior):
            self.ppc = [PriorPlots(prior_comp, figsize, save, path) for prior_comp in prior.priors]
    ...
    def plot_model(self, title='Fitted GP model as prior', file_name='fitted_GP_prior', ext='.pdf', with_latent=True, **kw):
        ...
        self.prior.model.plot(plot_limits=tuple(zip(*self.prior.bounds)), **kw)
        if title:
            plt.title(title)
        if self.save:
            plt.savefig(self.path + file_name + ext)
        if with_latent:
            self.prior.model.plot_latent(plot_limits=tuple(zip(*self.prior.bounds)), **kw)
            if title:
    ...
    file_name='fitted_GP_prior_contour', ext='.pdf', latent=True, **kw):
        ...
        model = kw.get('model', self.prior.model)
        ...
        plt.colorbar(cf1)
        if scatter:
            ax1.scatter(model.X.take([0], 1), model.X.take([1], 1), color='black')
        ...
        if scatter:
            ax2.scatter(model.X.take([0], 1), model.X.take([1], 1), color='black')
        plt.tight_layout()
